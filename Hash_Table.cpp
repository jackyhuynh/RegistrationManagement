 ...
 **
 *  Hash_Table::_reHashing
 *  RE-ALLOCATE A BIGGER TABLE IF THE LOAD FACTOR IS TOO HIGH(75%)
 *
 *  @param      - NONE
 *  @return     - VOID
 * // CHECK LOAD FACTOR SEE IF IT'S OVER 75%
 * if(occupied_count/static_cast<double>(tableSize)>0.75){
 * _reHashing();
 * }
 *
 void Hash_Table::_reHashing()
 {
 // PRINTING RE-HASHING STATUS
 cout << "*** RE-HASHING EXECUTES ***"                      << endl;
 cout << "[OLD TABLE SIZE]  : " << tableSize                << endl;
 cout << "[OLD OCCPY COUNT] : " << occupied_count           << endl;
 cout << "[OLD LOAD FACTOR] : " << setprecision(2) << fixed << showpoint
 << (occupied_count / static_cast<double>(tableSize)) * 100
 << endl;
 cout << "[OLD VALID COUNT] : " << valid_count              << endl;
 cin.sync();
 cin.get();
 
 // FIRST DECLARE THE NECCESSARY LOCAL VARIABLES
 int old_tableSize, new_tableSize;
 int new_occpyCount = 0;
 int new_full_bCount = 0;
 int new_validCount = 0;
 int new_overflow = 0;
 int new_delCount = 0;
 int new_collision = 0;
 
 old_tableSize = tableSize;
 new_tableSize = _get_prime_number(old_tableSize);
 
 cout << "[tableSize] : " << tableSize     << endl;
 cout << "[old size]  : " << old_tableSize << endl;
 cout << "[new size]  : " << new_tableSize << endl;
 cin.sync();
 cin.get();
 
 // DYNAMICALLY ALLOCATE NEW ARRAY
 hash_bucket *newTable = new hash_bucket[new_tableSize];
 for(int i=0; i<new_tableSize; i++){
 newTable[i].occupied_bucket = 0;
 for(int j=0; j<3; j++){
 newTable[i].bucket[j] = NULL;
 }
 }
 
 // INSERT FROM OLD TABLE TO NEW TABLE
 for(int i=0; i<old_tableSize; i++){
 if(hashTable[i].occupied_bucket != 0){
 for(int j=0; j<hashTable[i].occupied_bucket; j++){
 int new_hKey  = _get_hashKey(hashTable[i].bucket[j]->_get_studentID());
 Student *temp = hashTable[i].bucket[j];
 
 if(newTable[new_hKey].occupied_bucket == DEFAULT_BUCKET_SIZE){
 // FULL BUCKET
 cout << endl;
 cout << "---------------------------------------------------" << endl;
 cout << "[BUCKET FULL - " << new_hKey << " ] : DUMPING INPUT!"
 << endl;
 //out << *sdt_in << endl;
 cout << temp->_get_studentID() << endl;
 cout << "---------------------------------------------------" << endl;
 cout << endl;
 new_overflow++;
 }
 else{ // BUCKET IS NOT FULL
 if(newTable[new_hKey].occupied_bucket == 0){  // EMPTY BUCKET
 newTable[new_hKey].bucket[0] = temp;
 new_occpyCount++;
 newTable[new_hKey].occupied_bucket++;
 }
 else{ // BUCKET IS NOT EMPTY NOR FULL
 newTable[new_hKey].bucket[newTable[new_hKey].occupied_bucket]
 = temp;
 if(newTable[new_hKey].occupied_bucket == 2){  // WILL BE FULL
 newTable[new_hKey].occupied_bucket++; // INCREMENT TO 3
 new_full_bCount++;
 }
 else{ // STILL NOT FULL
 newTable[new_hKey].occupied_bucket++;
 ...
 occupied_count    = new_occpyCount;
 full_bucket_count = new_full_bCount;
 valid_count       = new_validCount;
 overflow_count   += new_overflow;
 delete_count      = new_delCount;
 collision_count   = new_collision;
 ...
 // PRINTING RE-HASHING STATUS
 cout << "*** AFTER RE-HASHING ***"                         << endl;
 cout << "[NEW TABLE SIZE]  : " << tableSize                << endl;
 cout << "[NEW OCCPY COUNT] : " << occupied_count           << endl;
 cout << "[NEW LOAD FACTOR] : " << setprecision(2)          << fixed
 << showpoint
 << (occupied_count / static_cast<double>(tableSize)) * 100
 << endl;
 cout << "[NEW VALID COUNT] : " << valid_count              << endl;
 cin.sync();
 cin.get();
 }
